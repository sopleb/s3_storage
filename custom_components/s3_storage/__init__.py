...
class S3StorageConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def validate_config(
        ...
        try:
            ...
            async with session.client(
                ...
                await s3_client.head_bucket(Bucket=config[CONF_BUCKET_NAME])
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            if not errors:
                service_name = user_input.get(CONF_ENDPOINT_URL, "AWS S3")
                title = f"{service_name}/{user_input[CONF_BUCKET_NAME]}"
